...
import { Card, CardBody, Spinner, Image, Chip, CardFooter } from "@heroui/react";
  ...
  if (isPending) {
    return (
      <div className="flex h-full items-center justify-center">
        <Spinner variant="wave" size="lg" label="Looking for episodes..." color="warning" />
      </div>
    );
  }
...
const EpisodeListCard: React.FC<EpisodeCardProps> = ({ episode, order = 1, id }) => {
  ...
  return (
    <Card
      ...
      shadow="none"
      className={cn(
        "group motion-preset-blur-right grid grid-cols-[auto,1fr] gap-3 border-2 border-foreground-200 bg-foreground-100",
        !isNotReleased && "hover:border-warning hover:bg-foreground-200",
        isNotReleased && "cursor-not-allowed opacity-50",
        order % 2 === 0 ? "motion-preset-blur-left" : "motion-preset-blur-right",
      )}
    >
      <div className="relative">
        ...
        {isNotReleased && (
          <Chip size="sm" color="warning" variant="shadow" className="absolute right-2 top-2 z-20">
            Coming Soon
          </Chip>
        )}
        <Chip
...
const EpisodeGridCard: React.FC<EpisodeCardProps> = ({ episode, id }) => {
  ...
  return (
    <Card
      ...
      shadow="none"
      className={cn(
        "group motion-preset-focus border-2 border-foreground-200 bg-foreground-100",
        !isNotReleased && "hover:border-warning hover:bg-foreground-200",
        isNotReleased && "cursor-not-allowed opacity-50",
      )}
    >
