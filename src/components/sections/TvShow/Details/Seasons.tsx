...
const TvShowsSeasonsSelection = forwardRef<HTMLElement, Props>(({ id, seasons }, ref) => {
  ...
  const [seasonNumber, setSeasonNumber] = useState(FILTERED_SEASONS[0].season_number.toString());
      ...
      <Card className="p-3">
        <CardHeader className="grid grid-cols-1 grid-rows-[1fr,auto] gap-3 md:grid-cols-[1fr,1fr,auto,auto]">
          <Select
            ...
            classNames={{
              trigger: "border-2 border-foreground-200",
            }}
          >
          ...
          <Input
            ...
            startContent={<Search />}
            classNames={{
              inputWrapper: "border-2 border-foreground-200",
            }}
          />
        ...
        <CardBody>
          <ScrollShadow className="h-[600px] py-2 pr-3">
