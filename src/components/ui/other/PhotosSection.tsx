        ...
        {images.slice(0, 4).map(({ file_path }, index) => (
          <div key={file_path} className="group relative">
            ...
            {index === 3 && images.length > 4 ? (
              <div
                className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center rounded-medium bg-black/40 text-xl font-bold text-white backdrop-blur-sm"
                onClick={() => setIndex(index)}
              >
                +{images.length - 4}
              </div>
